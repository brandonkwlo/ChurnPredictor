...
xgboost_model = load_model('app/models/xgb_model.pkl')
naive_bayes_model = load_model('app/models/nb_model.pkl')
random_forest_model = load_model('app/models/rf_model.pkl')
decision_tree_model = load_model('app/models/dt_model.pkl')
svm_model = load_model('app/models/svm_model.pkl')
knn_model = load_model('app/models/knn_model.pkl')
voting_classifier_model = load_model('app/models/voting_clf.pkl')
xgboost_SMOTE_model = load_model('app/models/xgboost-SMOTE.pkl')
xgboost_featureEngineered_model = load_model('app/models/xgboost-featureEngineered.pkl')
...
def explain_prediction(probability, input_dict, surname):
  ...
  Your machine learning model has predicted that a custer named {surname} has a {round(probability * 100, 1)}% probability of churning, based on the information provided below.property
  ...
  Don't mention the probability of churning, or the machine learning model, or say anything like "Based on the machine learning model's predition and top 10 most important features", just explain the prediction.
...
def generate_email(probability, input_dict, explanation, surname):
  ...
  Generate an email to the customer based on their information asking them to stay if they are at risk of churning, or offering these incentives so that they become more loyal to the bank.
...
df = pd.read_csv("app/data/churn.csv")
